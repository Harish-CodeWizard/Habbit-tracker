  ...
  return (
    <div className="px-4 py-2 flex flex-col gap-4">
      <div className="flex gap-2 mb-2">
        {['Evening', 'Morning'].map((label, i) => (
          <button key={label} className={`px-3 py-1 rounded-full text-xs font-semibold border ${i === 0 ? 'border-[#39ff14] text-[#39ff14]' : 'border-[#b0b3b8] text-[#b0b3b8]'}`}>{label}</button>
        ))}
      </div>
      <div className="flex gap-2 mb-2">
      ...
      {habits.map((habit, idx) => (
        <div key={idx} className="bg-[#18191A] border border-[#232526] rounded-2xl p-4 mb-2 shadow-[0_0_8px_#6c63ff33]">
