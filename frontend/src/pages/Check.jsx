  ...
  return (
    <div className="grid grid-cols-7 gap-1 mt-2">
      {grid.map((val, i) => (
        <div
          key={i}
          className={`w-7 h-7 flex items-center justify-center rounded-md border text-xs ${val ? 'bg-[#39ff14] text-black border-[#39ff14]' : 'bg-[#232526] text-[#b0b3b8] border-[#232526]'}`}
        >
          {val ? val : ''}
        </div>
      ))}
      ...
      <main className="flex-1 px-4 py-2 overflow-y-auto">
        ...
        {habits.map((habit, idx) => (
          <div key={idx} className="bg-[#18191A] border border-[#232526] rounded-2xl p-4 mb-4 shadow-[0_0_8px_#6c63ff33]">
            ...
            <HabitMonthGrid grid={habit.grid} />
          </div>
        ))}
      </main>
